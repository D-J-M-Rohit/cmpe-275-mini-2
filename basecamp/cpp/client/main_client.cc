...
static std::string GetArg(const std::string &key, int argc, char **argv,
...
int main(int argc, char **argv) {
  ...
  try {
    // reuse loader
    NodeCtx dummy = LoadNodeContext(
        tf_env, "A"); // will throw if no A; fine for sample topologies
    (void)dummy;
  } catch (...) { /* ignore */
  }
  ...
  if (!leader) {
    std::cerr << "No leader in topology\n";
    return 2;
  }
  ...
  if (target == "GREEN")
    team = basecamp::TEAM_GREEN;
  else if (target == "PINK")
    team = basecamp::TEAM_PINK;

  std::string addr = Addr(*leader);
  auto channel = grpc::CreateChannel(addr, grpc::InsecureChannelCredentials());
  auto stub = basecamp::Basecamp::NewStub(channel);

  std::string req_id = GetArg("request_id", argc, argv, "");
  if (req_id.empty()) {
    // Generate random request ID
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<uint64_t> dis;
    req_id = "req-" + std::to_string(dis(gen));
  }
  ...
  std::cout << "Sending request " << req_id << " to " << addr << "\n";
